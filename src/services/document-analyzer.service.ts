...
const documentCategorizationCache = new Map<string, DocumentCategory>();
...
class DocumentAnalyzerService {
   ...
   */
  public async categorizeDocumentWithAI(
...
Respond strictly as JSON with keys: {"category": string, "pvaraCategory": string, "subcategory": string, "confidence": number (0-1), "notes": string}.`;
    ...
    try {
      ...
      const category = (parsed.category as DocumentCategory['category']) || 'other';
      ...
      const pvaraCategory = parsed.pvaraCategory as DocumentCategory['pvaraCategory'] || 'supporting-document';
   ...
   */
  public categorizeDocument(filename: string): DocumentCategory {
   ...
   */
  public analyzeDocuments(applicationId: string, documents: string[]): DocumentAnalysis {
  ...
  public async analyzeWithAI(
    ...
    companyName: string,
    documents: string[],
    applicationType: string = 'VASP NOC'
  ): Promise<string> {
    if (!config.OPENAI_API_KEY) {
